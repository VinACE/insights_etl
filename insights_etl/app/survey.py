...
col2fld = {
    ...
    #'survey'       : (["Year"], 'string'), this is derived from the file name
    ...
    for field in surveys[survey_name]['header']:
        if field in field_map.keys():
            ...
            if field == 'survey':
                options = [survey_name]
            if field == 'category':
                options = ['Personal Wash', 'Home Care', 'Fabric Care', 'Hair Care', 'Toiletries']
    ...
    for field in surveys[survey_name]['header']:
        if field not in data:
            ...
            if variable_name in request.POST:
                data[field] = request.POST[variable_name]
    pass
